...
OKCancelWindow::OKCancelWindow(int width, int height, const char *title) :
		Fl_Window(width, height, title) {
	this->buttonInvoked = WindowResult::NONE;

	begin();

	this->okButton = new Fl_Button(0, 0, 70, 30, "Check");
	this->okButton->callback(cbOk, this);

	this->cancelButton = new Fl_Button(0, 0, 70, 30, "Quit");
...
void OKCancelWindow::setWindowResult(OKCancelWindow::WindowResult result) {
