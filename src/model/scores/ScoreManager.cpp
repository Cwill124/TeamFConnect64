...
void ScoreManager::addScore(string &playerName, int time, int puzzle) {
	...
	if (sortedScores.size() < Settings::MaximumHighScores) {
		this->scores.push_back(
				new Score(this->scores.size(), playerName, time, puzzle));
	} else {
		for (Score *score : sortedScores) {
			if (score->getTime() > time) {
				this->removeScore(score->getId());
				this->scores.push_back(
						new Score(this->scores.size(), playerName, time,
								puzzle));
				return;
			}
		}
...
void ScoreManager::removeScore(int idToRemove) {
	for (auto iterator = this->scores.begin(); iterator != this->scores.end();
			iterator++) {
...
vector<Score*> ScoreManager::getScoresSortedByTime() {
	...
	std::sort(sortableScores.begin(), sortableScores.end(),
			compareByTimeAscending);
